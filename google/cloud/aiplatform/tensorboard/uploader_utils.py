...
class OnePlatformResourceManager(object):
    ...
    def batch_create_runs(
        ...
        self._run_name_to_run_resource_name.update(
            {run.display_name: run.resource_name for run in created_runs}
            )
    ...
    def _create_or_get_run_resource(
        ...
        """
        exp_run = experiment_run_resource.ExperimentRun.create(
                run_name=run_name,
                experiment=self._experiment_resource_name.split("/experiments/")[1],
                tensorboard=self._experiment_resource_name.split("/experiments/")[0],
                state=gca_execution.Execution.State.STATE_UNSPECIFIED,
            )
        tb_run_artifact = exp_run._backing_tensorboard_run
